...
export const jobWorker = new Worker(
  ...
  async (job) => {
    ...
    const { jobId, action, type, scheduledAt, intervalMinutes, payload } = job.data;
    ...
    try {
      ...
      if (type === "scheduled") {
        ...
        const { result, capturedLogs } = await runTask(action, payload);
...
async function runTask(action: string, payload: any) {
  ...
  try {
    ...
    switch (action) {
      ...
      case "analyze_coin_flows":
        ...
        result = await analyzeCoinFlows(payload.tokenAddress, payload.options);
        break;
        
      case "analyze_nft_movements":
        console.log("🖼️ Analyzing NFT movements for collection:", payload.collectionAddress);
        result = await analyzeNFTMovements(payload.collectionAddress, payload.options);
        break;
